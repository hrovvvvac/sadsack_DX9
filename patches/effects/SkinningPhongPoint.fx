...
VSOutput VS(VSInput input)
{
	...
	float3 posN = mul(pos, tW);
	float3 LightDirV = normalize(lPos-posN);
	output.LightDirV = mul(LightDirV, tV);
	
	//normal in view space
	output.NormV = normalize(mul(norm2, tWV));
	...
	//output.PosW = mul(PosO, tW);
	output.PosW = mul(pos, tW);
...
float4 PS(VSOutput input)  : COLOR
{
    ...
    col.rgb *= PhongPoint(input.PosW, input.NormV, input.ViewDirV, input.LightDirV);
	col.a *= Alpha;
